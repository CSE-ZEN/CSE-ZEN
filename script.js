...
// SEO: normalize extensionless top-level URLs generated by GitHub Pages.
// This avoids duplicate crawl paths such as /notre-offre and /notre-offre.html.
(() => {
  const canonicalHtmlPages = new Set([
    'contact',
    'equipe-reseau',
    'formations',
    'notre-offre',
  ...
  const isTopLevelCleanUrl = path === `/${lastSegment}` && canonicalHtmlPages.has(lastSegment);

  if (isTopLevelCleanUrl) {
    window.location.replace(`/${lastSegment}.html${window.location.search}${window.location.hash}`);
  }
})();
